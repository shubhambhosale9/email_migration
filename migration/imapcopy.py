...
class IMAP_Copy(object):
	...
	try:
            for source_mailbox, destination_mailbox in self.mailbox_mapping:
                log_file.write("Started folder migration from =>> %s =to=>> %s" %(source_mailbox, destination_mailbox))
		    ...
		    while not connected:	
			    ...
			    if "[AUTHENTICATIONFAILED] Invalid credentials (Failure)" in str(e):
			        print("please check your login details its wrong")
				break
                            else:		
                                print("Connection not establish error caught :- " + str(e))
                                print("waitting")
			        for i in reversed(range(0, 60)):
                                    time.sleep(1)
                                    print ("waiting for connecting to account:-" + str(i))	
                      
		    connected = False	      	  
		    coun = count + 1	  		 
                    new_copy_count, mail_count = self.copy(source_mailbox, destination_mailbox, self.skip, self.limit)
		    if count == 1:
		        if self.skip != 0:
			    self.skip = self.skip + new_copy_count + 1   	
                            self.logger.info("skipping mail till =>> %d and continue" %(self.skip))
			else:
		            self.skip = new_copy_count + 1
                            if self.skip < mail_count :
                                self.logger.info("%d mails gets copyied out of %d we skipping mail till %d and continue\n"                                                                 %(self.skip, mail_count, self.skip))

                                log_file.write("%d mails gets copyied out of %d we skipping mail till %d and continue\n"                                                                   %(self.skip, mail_count, self.skip))
		    else: 	
		        self.skip = self.skip + new_copy_count + 1
                        if self.skip < mail_count :
                            self.logger.info("%d mails gets copyied out of %d we skipping mail till %d and continue\n"                                                                %(self.skip, mail_count, self.skip))
		    ...
		    copy_count = self.skip
                    
            if copy_count >= mail_count :
                log_file.write("Folder migrated sucessfully from =>> %s =to=>> %s\n\n\n" %(source_mailbox, destination_mailbox))    

	except Exception as e:
            log_file.write("Exception caught\n")
            log_file.write("the folder from which the mails copy from =>> %s =to=>> %s\n" %(source_mailbox, destination_mailbox))
            log_file.write("the copy count id %d and remaining mail count is %d\n" %(copy_count, mail_count))
	    self.logger.info("Exception Caught in Run:- %s" %(e)) 		
...
def main():
    ...
    log_file = open(""+source_auth[0]+".txt", "a+")
